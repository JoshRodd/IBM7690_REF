...
SERVICES display-menu resources at2800h list self-test, attributes, character set,80×25,40×25,320×200,640×200/640×480, screen paging, exit, and `10 - RUN ALL ABOVE TESTS`. They ask whether all8 pages displayed and whether the screen is correct; the strings do not establish a reachable customer-level selection menu. **Human visual responses are part of the oracle.** See [VIDEO.md](VIDEO.md) for the actual MCGA/LCD mapping; do not count a correctly drawn diagnostic menu as proof of individual pixel/page/palette behavior.

**BINARY, loose v1.10:** DISPLAY1's graphics fill table at1282h is `55 55 AA AA FF FF`. In mode04 these produce solid two-bit pels01,10,11. The reference's C1-only rule predicts black, white, white; the emulator intentionally overrides that rule with midpoint gray, midpoint gray, on to satisfy the requested rendering model. The DAC ramp at12A8h (`00 00, 02 16, 04 2B, 06 3F`) affects the analog CRT path, not the pre-DAC LCD path. In one-bit mode06, the same55h/AAh/FFh bytes instead produce two phase-shifted spatial dither patterns and solid white.

**CONTROL FLOW:** DISPLAY1 classifies BDA40:89 bits4 and2 (font size and monochrome/color-map state), not a special LCD display-combination code. Its monochrome branch changes palette/message handling and enables the later mode11 test; it does not omit mode04. The observed normal `SERVICES 0` route runs the display sequence through640×480 and returns to device checkout without exposing an individual display menu or a paging prompt.

**ORIGINAL HARDWARE:** the [physical 7690 diagnostic photograph](https://www.ardent-tool.com/7690/7690-4.jpg), credited to Santo Nucifora by [Ardent Tool](https://www.ardent-tool.com/7690/), visibly shows `640X480 GRAPHICS - GRAY, GRAY, WHITE ON BLACK` with two striped regions and a solid region. It corroborates spatial dithering in that mode, not native gray levels or the physical320×200 outcome.

**LCD CHECKOUT ACCEPTANCE:** for the modeled7690 LCD, mode04 must now render both01 and10 at the arithmetic RGB average of the selected off/on pixels, with00 off and11 on. Both gray boxes must actually be visible; the former black/white/white acceptance exception no longer applies. Preserve the original diagnostic bytes. This explicit departure from the Technical Reference is an emulation assumption, not proof of original-hardware grayscale capability.
